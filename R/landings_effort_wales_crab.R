# script for processing MMO/WG (IFISH) vessel-level landings, effort data of the welsh stock of brown crab
...
ifish_landings_data <- readr::read_csv(file = "All Species caught in WW by vessels who have caught CRE,LBE,WHE in same period - iFish SN views.csv", 
  ...
  dplyr::filter(SPECIES_CODE %in% c("CRE")) |> 
...
(plot2 <- ifish_landings_data |> 
    ...
    dplyr::mutate(vessel_len_cat = factor(vessel_length_category, 
                                                  levels = c("Under 10", "10 to 12", "12 to 15", "Over 15"))) |>
    dplyr::filter(species_code == "CRE") |> 
    #dplyr::filter(gear_code %in% c("GEN", "GN", "GNS")) |>
    #dplyr::filter(gear_code %in% c("OT", "TBB")) |>
...
ifish_landings_data_crab <- ifish_landings_data |> 
...
# save output
readr::write_csv(ifish_landings_data_crab, file = "ifish_nominal_catch_pot_crab_clean.csv") 

# reformat landing time series for ss - set for annual time step
# Catch data: yr, season, fleet, catch, catch_se
ifish_landings_wales_crab_yr <- ifish_landings_data_crab |>
...
# export datasets
readr::write_csv(ifish_landings_wales_crab_yr, file = "landing.data_crab_wales_ss-format_yr.csv") 
