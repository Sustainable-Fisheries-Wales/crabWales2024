# script for standardizing cpue from the observer sampling of the welsh stock of edible crab (inshore waters only)
...
source(file = "compute_cpue_wales_crab.R" )

# read in data
observer_data_crab <- readr::read_csv("observer_data_crab_nominal.cpue_potset.csv",
                                         col_types = readr::cols(ices_sub_rect = readr::col_character())) |> 
  dplyr::glimpse()

# reformat datasets
observer_data_orig <- observer_data_crab
stock <- "crab"
...
covariate_data <- observer_data_orig |>
  ...
  dplyr::mutate(areaSwept_km2 = 1) |>
  dplyr::arrange(year, month) |>
  dplyr::select(year, latitude, longitude, fisher, month, quarter, ices_rect, nominal.cpue_potset_lobster, escape_gaps,
...
# plot
ifish_data_yr |> ggplot2::ggplot(ggplot2::aes(x=as.numeric(year), y=obj)) +#, group = std, color = std)) +  
...
(plot2 <- ggplot2::ggplot() +  
    ...
    ggplot2::geom_point(data=data, 
                        ggplot2::aes(x=longitude, y=latitude, size = nominal.cpue_potset), shape = 21, fill="darkred", color="darkgray", 
                        alpha=I(0.3)) + 
    ggplot2::theme_classic() +
...
settings <- FishStatsUtils::make_settings(n_x = 1000, #1=encounter & 2=density
                                          ...
                                          RhoConfig = c("Beta1" = 4, "Epsilon1" = 0, "Beta2" = 4, "Epsilon2" = 0),
                                          ObsModel = c(5, 0), 
...
covariate_data[,'nominal.cpue_potset_lobster'] = (covariate_data[,'nominal.cpue_potset_lobster'] - 
                                                 mean(unlist(covariate_data[,'nominal.cpue_potset_lobster'])))/sd(unlist(covariate_data[,'nominal.cpue_potset_lobster']))
catchability_data[,'nominal.cpue_potset_lobster'] = (catchability_data[,'nominal.cpue_potset_lobster'] - 
                                                    mean(unlist(catchability_data[,'nominal.cpue_potset_lobster'])))/sd(unlist(catchability_data[,'nominal.cpue_potset_lobster']))

# Define formula
# encounter rate
# catchability
Q1_formula = ~ escape_gaps 
# density/habitat
X1_formula = ~ month + nominal.cpue_potset_lobster 
Q1config_k = c(3) 
X1config_cp = array(3, dim=c(1, 2))

# positive catch rate
# catchability
Q2_formula = ~ escape_gaps 
# density/habitat
X2_formula = ~ month + nominal.cpue_potset_lobster 
Q2config_k = c(3) 
X2config_cp = array(3, dim=c(1, 2))

# make settings
dat <- catchability_data
settings <- FishStatsUtils::make_settings(n_x = i, # 1=encounter & 2=density
                                          FieldConfig = c("Omega1" = 1, "Epsilon1" = 1, "Omega2" = 1, "Epsilon2" = 1),
                                          RhoConfig = c("Beta1" = 4, "Beta2" = 3, "Epsilon1" = 4, "Epsilon2" = 2),
                                          ObsModel = c(5, 0), 
                                          OverdispersionConfig = c("Eta1" = 1, "Eta2" = 0), 
                                          Options = c('treat_nonencounter_as_zero' = FALSE, "report_additional_variables" = TRUE), 
                                          Region='User',
